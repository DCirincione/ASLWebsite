...
export default function EventsPage() {
  ...
  const isAldrichEvent = (event: EventItem) => {
    const inTitle = event.title?.toLowerCase().includes("aldrich");
    const inLocation = event.location?.toLowerCase().includes("aldrich");
    return Boolean(inTitle || inLocation || event.status === "scheduled");
  };
  ...
  const prepareCollections = () => {
    ...
    const aldrich = ordered.filter(isAldrichEvent).slice(0, 6);
    const featuredFallback = ordered.slice(0, 4);
    const featured = ordered.filter((ev) => ev.status !== "tbd").slice(0, 4);

    return {
      aldrichEvents: aldrich.length > 0 ? aldrich : featuredFallback,
      featuredEvents: featured.length > 0 ? featured : featuredFallback,
      allEvents: ordered,
    };
  };

  const renderEventTile = (event: EventItem) => {
    const dateRange = formatDateRange(event.start_date, event.end_date);
    const timeInfo = event.time_info?.trim();
    const primaryDate = timeInfo || dateRange || "Date TBD";

    return (
      <article key={event.id} className="event-pill">
        <div className="event-pill__header">
          <h3>{event.title}</h3>
          <span className={statusClass(event.status)}>{statusLabel(event.status)}</span>
        </div>
        <div className="event-pill__meta">
          <span>{primaryDate}</span>
          {event.location ? <span>{event.location}</span> : <span>Location TBD</span>}
        </div>
        {event.description ? (
          <p className="event-pill__description">{event.description}</p>
        ) : null}
        <div className="event-pill__actions">
          ...
          <div className="events-deck">
            <div className="events-group">
              ...
              <div className="event-pill-grid">
                {collections.aldrichEvents.map(renderEventTile)}
              </div>
            </div>
            ...
            <div className="events-group">
              <div className="events-group__header">
                ...
                <p className="muted">Handpicked matchups and community favorites.</p>
              </div>
              <div className="event-pill-grid">
                {collections.featuredEvents.map(renderEventTile)}
              </div>
            </div>
            ...
            <div className="events-group">
              ...
              <div className="event-pill-grid">
                {collections.allEvents.map(renderEventTile)}
              </div>
            </div>
          </div>
            );
