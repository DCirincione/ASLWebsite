...
export default function EventsPage() {
  ...
  const activeCalendarMonth = useMemo(() => {
    ...
    return monthOptions.includes(currentMonth) ? currentMonth : monthOptions[0];
  ...
  const calendarCells = useMemo<CalendarCell[]>(() => {
    ...
    for (let day = 1; day <= daysInMonth; day += 1) {
      const date = new Date(Date.UTC(year, month - 1, day));
      cells.push({
        key: `day-${activeCalendarMonth}-${day}`,
        label: String(day),
        isEmpty: false,
        isCurrentMonth: true,
  ...
  const selectedMonthIndex = monthOptions.findIndex((entry) => entry === activeCalendarMonth);
