...
export default function PickleballPage() {
  ...
  useEffect(() => {
    const loadEvents = async () => {
      ...
      if (!error && !sportsError && data) {
        ...
        const pickleballOnly = filterVisiblePublicEvents(data as Event[]).filter((row) =>
          sportMatchesEvent(row, "pickleball", sports) &&
          (row.registration_program_slug ?? "").trim().toLowerCase().startsWith("pickleball-")
        );
        const withSignupCounts = await attachPublicEventSignupCounts(supabase, pickleballOnly);
  ...
  return (
    <PageShell>
      ...
      <SportSponsorBanner sportSlug="pickleball" />
      <Section
        id="pickleball-hero"
        eyebrow="Pickleball"
