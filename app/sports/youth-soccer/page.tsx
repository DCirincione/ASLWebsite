...
export default function YouthSoccerPage() {
  ...
  useEffect(() => {
    const loadEvents = async () => {
      ...
      if (!error && !sportsError && data) {
        ...
        const youthSoccerOnly = filterVisiblePublicEvents(data as Event[]).filter((row) => {
          const slug = (row.registration_program_slug ?? "").trim().toLowerCase();
          return (
            sportMatchesEvent(row, "youth-soccer", sports) &&
            (slug.startsWith("youth-soccer-clinic") || slug.startsWith("youth-soccer-league"))
          );
        });

        const withSignupCounts = await attachPublicEventSignupCounts(supabase, youthSoccerOnly);
  ...
  return (
    <PageShell>
      ...
      <SportSponsorBanner sportSlug="youth-soccer" />
      <Section
