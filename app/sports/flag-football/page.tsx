  ...
  useEffect(() => {
    const loadEvents = async () => {
      ...
      if (!error && !sportsError && data) {
        ...
        const flagFootballOnly = filterVisiblePublicEvents(data as Event[]).filter((row) => {
          const slug = eventSlug(row.registration_program_slug);
          return (
            sportMatchesEvent(row, "flag-football", sports) &&
            (
        ...
        const withSignupCounts = await attachPublicEventSignupCounts(supabase, flagFootballOnly);
