  ...
  useEffect(() => {
    const loadEvents = async () => {
      ...
      if (!error && !sportsError && data) {
        ...
        const miniGolfOnly = filterVisiblePublicEvents(data as Event[]).filter((row) => {
          const slug = (row.registration_program_slug ?? "").trim().toLowerCase();
          return (
            sportMatchesEvent(row, "mini-golf", sports) &&
            (slug.startsWith("mini-golf-league") || slug.startsWith("mini-golf-tournament"))
          );
        });
        const withSignupCounts = await attachPublicEventSignupCounts(supabase, miniGolfOnly);
